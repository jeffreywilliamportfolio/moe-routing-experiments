#!/usr/bin/env python3
"""
DeepSeek V3.1 — Strange Loop Paired Experiment (Prefill-Only).

60 prompts: 30 A ("this paradox/loop/structure") + 30 B ("a paradox/loop/structure").
Cal-Manip-Cal sandwich structure. Control experiment for selfref-paired-1.
Content is recursive/self-referential in the abstract sense but NOT about the model.
"""
import glob
import json
import os
...
OUTPUT_DIR = "output"
RESULTS_FILE = "results_strangeloop_paired_prefill_ds31.json"
...
def compute_metrics(prompt_dir, n_prompt):
    ...
    good_layers = sorted([layer_index for layer_index in shapes if shapes[layer_index] >= median_rows * 0.5])
...
def main():
    ...
    print("=== DeepSeek V3.1 — Strange Loop Paired Experiment ===")
    ...
    print("Content: Gödel, Escher, bootstrap paradoxes, quines, tangled hierarchies")
    print("Control for selfref-paired-1 (recursive but NOT about the model)")
    print()
    ...
    output = {
        "experiment": "ds31_strangeloop_paired_1",
