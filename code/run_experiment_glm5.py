#!/usr/bin/env python3
"""
GLM-5 (745B MoE) -- Self-Referential 3-Condition Experiment (Prefill-Only).

90 prompts: 30 A (this system) + 30 B (a system) + 30 C (your system).
Cal-Manip-Cal sandwich structure.

Architecture: 256 experts, top-8 routing, 75 MoE layers (L3-L77).
Entropy normalized by log2(256).
"""
import glob
import json
import os
...
MODEL = os.environ.get(
    "MODEL_PATH",
    "/workspace/models/GLM-5-UD-Q2_K_XL/GLM-5-UD-Q2_K_XL-00001-of-00007.gguf",
)
...
RESULTS_FILE = "results_selfref_3cond_prefill_glm5.json"
...
# GLM-5: 256 experts, top-8, 75 MoE layers (L3-L77)
# Layer 77 has truncated rows (1 vs n_tokens) — analogous to DeepSeek L57
EXCLUDED_LAYERS = {77}
...
def compute_metrics(prompt_dir, n_prompt):
    ...
    if not EXCLUDED_LAYERS and excluded_layers:
...
def main():
    ...
    print("=== GLM-5 (745B MoE) -- Self-Referential 3-Condition Experiment ===")
    ...
    print("Cal-Manip-Cal sandwich, cold KV cache, GLM template")
    print()
    ...
    output = {
        "experiment": "glm5_selfref_3cond_1",
        ...
        "chat_template": "[gMASK]<sop><|user|>{prompt}<|assistant|>",
