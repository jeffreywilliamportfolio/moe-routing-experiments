#!/usr/bin/env bash
# setup_instance.sh — Bootstrap 4× H200 Vast.ai instance for Ling-1T capture.
#
# Run this script ON THE INSTANCE after SSH in:
#   bash /workspace/experiment-ling1t/setup_instance.sh
#
# Phases:
...
#   D — token_verify, then main run
#   E — print download command, then kill instance from local machine
#
# All output is tee'd to /workspace/experiment-ling1t/setup.log

set -euo pipefail
WORKDIR=/workspace/experiment-ling1t
LOG="$WORKDIR/setup.log"
mkdir -p "$WORKDIR"
exec > >(tee -a "$LOG") 2>&1

echo "============================================================"
echo " Ling-1T Instance Bootstrap — $(date)"
echo "============================================================"
...
# Poll until download complete marker appears
...
echo "If tensor name differs, update is_router_tensor() locally, re-upload, rebuild."

# ----------------------------------------------------------------
# PHASE D — token_verify + main run
# ----------------------------------------------------------------
echo ""
echo "=== PHASE D: Generate TSV ==="
cd "$WORKDIR"
python3 generate_tsv.py --suite prompt_suite.json
...
echo "    root@HOST:/workspace/experiment-ling1t/output/ \\"
echo "    experiments/ling1t-selfref-paired-1/"
echo ""
echo "  scp -P PORT -i ~/.ssh/vast_16 \\"
echo "    root@HOST:/workspace/experiment-ling1t/{experiment.log,build_commit.txt,binary_md5.txt,tensor_names.txt} \\"
echo "    experiments/ling1t-selfref-paired-1/"
echo ""
echo "  Then kill the instance and run:"
echo "    python3 experiments/ling1t-selfref-paired-1/analyze_local.py \\"
echo "      --output-dir experiments/ling1t-selfref-paired-1/output/ \\"
echo "      --prompt-suite experiments/ling1t-selfref-paired-1/prompt_suite.json"
echo ""
echo "============================================================"
