...
#   bash /workspace/experiment-ling1t-cat/setup_instance.sh
#
# Phases:
...
set -euo pipefail
WORKDIR=/workspace/experiment-ling1t-cat
LOG="$WORKDIR/setup.log"
mkdir -p "$WORKDIR"
exec > >(tee -a "$LOG") 2>&1

echo "============================================================"
echo " Ling-1T 5-Cond Cat Instance Bootstrap — $(date)"
echo "============================================================"
...
echo "    root@HOST:/workspace/experiment-ling1t-cat/output/ \\"
echo "    experiments/ling1t-5cond-cat-1/"
echo ""
echo "  scp -P PORT -i ~/.ssh/vast_16 \\"
echo "    root@HOST:/workspace/experiment-ling1t-cat/{experiment.log,build_commit.txt,binary_md5.txt,tensor_names.txt,setup.log} \\"
echo "    experiments/ling1t-5cond-cat-1/"
echo ""
echo "  Then kill the instance and run:"
echo "    python3 experiments/ling1t-5cond-cat-1/analyze_local.py \\"
echo "      --output-dir experiments/ling1t-5cond-cat-1/output/ \\"
echo "      --prompt-suite experiments/ling1t-5cond-cat-1/prompt_suite.json"
echo ""
echo "============================================================"
